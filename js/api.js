...
var base_url_standing_ENG = "https://api.football-data.org/v2/competitions/2021/standings?standingType=TOTAL";
    ...
    caches.match(base_url + fl_teams).then(function (response) {
      if (response) {
        response.json().then(function (data) {
          var temasHTML = "";
          var loading = document.getElementById("anim-loding");
          if (data <= 0) {
                ...
                <div class="col s12">
                  <div class="card-panel blue">
                    <span class="white-text ">
                      <i class="tiny material-icons">info_outline</i> No data result
                    </span>
                  </div>
                </div>
              </div>`;
...
function getStandingComp() {
  // jika tersimpan di chace
  if ('caches' in window) {
    caches.match(base_url_standing_ENG).then(function (response) {
      if (response) {
        response.json().then(function (data) {
          // Objek/array JavaScript dari response.json() masuk lewat data.
  ...
  fetch(base_url_standing_ENG, token)
    .then(status)
    .then(json)
    .then(function (data) {
      console.log(data);
...
function getsvd_teamsByid() {
  ...
  getById_tmSVD(idParam).then(function (teams) {
    ...
    var loading = document.getElementById("anim-loding");
    var bodyTabel = document.getElementById("body-tabel");
    var noData = document.getElementById("data-empty");
    var datatabel_comp = "";
    var datatabel_play = "";
    var articleHTML = "";
    var no_play = 1;
